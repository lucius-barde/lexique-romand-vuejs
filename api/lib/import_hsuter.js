...
function normalizeCategory(category) {
  ...
  if (/verbe|\bv\b/.test(value)) return 'Verbe'
  if (/adjectif|\badj\b/.test(value)) return 'Adjectif'
  if (/adverbe|\badv\b/.test(value)) return 'Adverbe'
  if (/interjection|\binterj\b/.test(value)) return 'Interjection'
  if (/locution/.test(value)) return 'Locution'
  if (/expression/.test(value)) return 'Expression'
  if (/nom|\bn\.?\b/.test(value)) return 'Nom'
  return 'Autre'
}
...
export async function searchHsuter(searchTerm) {
  ...
  while ((match = entryRe.exec(html))) {
    ...
    best = { score: localBest.score, result: { term, category: normalizeCategory(extractCategory(match[1])), definition, example, variants, source_id: 'hsuter', source: `${url}#${encodeURIComponent(localBest.anchor.name)}` } };
